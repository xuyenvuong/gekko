...
method.check = function(candle) {
  ...
  if (!this.trend.adviced) {
    if (this.trend.lastAdvice == 'long') {
      ...
      } else if (cs.isDoji(candle) && cs.isAllBullish(trendByDuration) && p > 0.25) { // TODO: update const
        this.setTrend('short', cs.isBearish(candle) ? 1 : 0);
        log.debug('  <<<<<<<<<<<<<<<<<<<<<<<< SELL SELL SELL Doji #2', candle.close.toFixed(d), 'pl:', this.pl += candle.close);
      }
    } else if (this.trend.lastAdvice == 'short') {
      if (cs.isDoji(candle) && cs.isBearish(lastCandle) && cs.isBearish(b) && p > 0.5) { // TODO: update const
        this.setTrend('long', cs.isBullish(candle) ? 1 : 0);
        log.debug('  >>>>>>>>>>>>>>>>>>>>>>>> BUY BUY BUY Doji #1', candle.close.toFixed(d), 'pl:', this.pl -= candle.close);
      } else if (cs.isDoji(candle) && cs.isAllBearish(trendByDuration) && p > 0.25) { // TODO: update const
        this.setTrend('long', cs.isBullish(candle) ? 1 : 0);
  ...
  if (!this.trend.adviced) {
    if (this.trend.lastAdvice == 'long') {
      ...
      if (cs.isBearishHarami(lastCandle, candle) && cs.calculateBodyPercentage(lastCandle) < 0.4) { // TODO: 0.1 + 0.3
  ...
  if (false && !this.trend.adviced) {
    if (this.trend.lastAdvice == 'long') {
      if (cs.isBullishEngulfing(lastCandle, candle) && cs.calculateBodyPercentage(lastCandle) < 0.4) { // TODO: 0.1 + 0.3
        this.setTrend('short', 0);
        log.debug('  <<<<<<<<<<<<<<<<<<<<<<<< SELL SELL SELL isBullishHarami #1', candle.close.toFixed(d), 'pl:', this.pl += candle.close);
      } else if (cs.isBearishHarami(lastCandle, candle)) { //TODO only if lower than short price
        this.setTrend('short', 0);
...
method.setTrend = function(advice, keep) {
  if (advice == 'long' && this.getTrendLastCandles().close >= this.trend.lastPrice && this.trend.lastPrice != -1) {
    log.debug('      ======== IGNORE #1 ==================');
    return;
  } else if (advice == 'short' && this.getTrendLastCandles().close <= this.trend.lastPrice && this.trend.lastPrice != -1) {
    log.debug('      ======== IGNORE #2 ==================');
    return;
  }
